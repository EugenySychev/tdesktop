...
QString LangDir() {
	return cWorkingDir() + "tdata/tgc_lang/";
}
...
void ParseLanguageData(
	...
	const auto filename = isDefault
		? qsl(":/tgc_lang/%1.json").arg(langCode)
	...
	if (!file.open(QIODevice::ReadOnly)) {
		LOG(("TGC::Lang Info: file %1 could not be read.").arg(filename));
		return;
	}
	...
	if (error.error != QJsonParseError::NoError) {
		LOG(("TGC::Lang Info: file %1 has failed to parse. Error: %2"
			).arg(filename
			).arg(error.errorString()));
		return;
	} else if (!document.isObject()) {
		LOG(("TGC::Lang Info: file %1 has failed to parse. Error: object expected"
			).arg(filename));
		return;
	}
		...
		if ((*value).isString()) {
			...
			for (auto pli = pluralList.constBegin(), ple = pluralList.constEnd(); pli != ple; ++pli) {
				...
				if (!(*pluralValue).isString()) {
					LOG(("TGC::Lang Info: wrong value for key %1 in %2 in file %3, string expected")
			...
			LOG(("TGC::Lang Info: wrong value for key %1 in file %2, string or object expected")
...
void UnpackDefault() {
	...
	const auto langs = QDir(":/tgc_lang").entryList(QStringList() << "*.json", QDir::Files);
	...
	for (auto language : langs) {
		...
		auto input = QFile(qsl(":/tgc_lang/%1.json").arg(language));
